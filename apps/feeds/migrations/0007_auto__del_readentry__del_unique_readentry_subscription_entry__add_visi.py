...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Removing unique constraint on 'ReadEntry', fields ['subscription', 'entry']
        db.delete_unique('feeds_readentry', ['subscription_id', 'entry_id'])

        # Deleting model 'ReadEntry'
        db.delete_table('feeds_readentry')

        # Adding model 'Visited'
        db.create_table('feeds_visited', (
        ...
        db.send_create_signal('feeds', ['Visited'])

        # Adding unique constraint on 'Visited', fields ['user', 'entry']
        db.create_unique('feeds_visited', ['user_id', 'entry_id'])


    def backwards(self, orm):
        
        # Removing unique constraint on 'Visited', fields ['user', 'entry']
        db.delete_unique('feeds_visited', ['user_id', 'entry_id'])

        # Adding model 'ReadEntry'
        db.create_table('feeds_readentry', (
            ('entry', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['feeds.Entry'])),
            ('id', self.gf('django.db.models.fields.AutoField')(primary_key=True)),
            ('marked', self.gf('django.db.models.fields.BooleanField')(default=False)),
            ('subscription', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['feeds.Subscription'])),
        ))
        db.send_create_signal('feeds', ['ReadEntry'])

        # Adding unique constraint on 'ReadEntry', fields ['subscription', 'entry']
        db.create_unique('feeds_readentry', ['subscription_id', 'entry_id'])

        # Deleting model 'Visited'
        db.delete_table('feeds_visited')
    ...
    models = {
        ...
        'feeds.visited': {
            'Meta': {'ordering': "('user', 'entry')", 'unique_together': "(('user', 'entry'),)", 'object_name': 'Visited'},
